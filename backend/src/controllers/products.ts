...
const createProduct = async (
  ...
  try {
    ...
    // Защита: проверка наличия fileName перед перемещением файла
  ...
  try {
    ...
    const product = await Product.findByIdAndUpdate(
      productId,
      {
        $set: {
          ...req.body,
          price: req.body.price ?? null, // Защита от undefined/null
          image: req.body.image ?? undefined, // Защита от undefined
        },
      },
    ...
    if (error instanceof MongooseError.CastError) {
      return next(
        new BadRequestError('Передан не валидный ID товара')
      );
    }
    if (
      error instanceof Error &&
